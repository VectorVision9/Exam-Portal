...
app.factory('ApplicationService', ['$q', function ($q) {
  ...
  function updateStatus(appId, status, extra) {
    return $q.when(
      db.collection('applications').doc(appId).update(angular.extend({ status: status }, extra || {}))
    );
  }
  ...
  function getApplicationById(appId) {
    return $q.when(
      db.collection('applications').doc(appId).get().then(function (doc) {
        return doc.exists ? angular.extend({ id: doc.id }, doc.data()) : null;
      })
    );
  }

  // Simple roll-number generator for a learning project.
  // Not guaranteed globally unique under heavy concurrent load —
  // a production system would use a Firestore transaction / counter
  // document instead. Good enough to demonstrate the admit-card flow.
